...
/**
 * ThaiJuan → FentonD Discord Notifier
 * 
 * Checks for pending Instagram post notifications and sends to FentonD via Discord webhook.
...
const LOG_PATH = '/tmp/thaijuan-notify-fenton.log';

// Discord webhook URL for FentonD direct chat
const WEBHOOK_URL = process.env.DISCORD_WEBHOOK_URL_FENTON || process.env.DISCORD_WEBHOOK_URL;
...
Posted at ${new Date().toLocaleTimeString('en-AU', { timeZone: 'Australia/Melbourne' })}`;
    ...
    fs.writeFileSync(path.join(ARCHIVE_PATH, `fenton-${Date.now()}.json`), JSON.stringify(data, null, 2));
